...
echo -e "${BLUE}=================================================${NC}"
echo -e "${GREEN}  🚀 欢迎使用 Ubuntu 全能交互式初始化脚本 🚀  ${NC}"
echo -e "${BLUE}=================================================${NC}"

# 确保脚本以 root 权限运行
if [ "$EUID" -ne 0 ]; then
  echo -e "${YELLOW}❌ 请使用 root 权限运行此脚本 (例如: sudo bash ubuntu_init.sh)${NC}"
  exit 1
fi

# 0. Ubuntu 专属优化：彻底禁用交互式弹窗 (防止 needrestart 和 dpkg 卡死脚本)
...
if [ -s "$USER_HOME/.ssh/authorized_keys" ]; then
    ...
    if [[ ! "$setup_ssh_key" =~ ^[Nn]$ ]]; then
        read -r -p "📝 请在此处粘贴您的 SSH 公钥 (例如 ssh-rsa/ssh-ed25519 开头): " ssh_pub_key
...
# Ubuntu 专属的静默升级参数，防止被配置冲突弹窗卡死
apt upgrade -y -q -o Dpkg::Options::="--force-confdef" -o Dpkg::Options::="--force-confold"
...
echo -e "\n${BLUE}=================================================${NC}"
echo -e "${GREEN}🎉 恭喜！Ubuntu 初始化流程全部完美结束！🎉${NC}"
if [ "$ACTUAL_USER" != "root" ]; then
